...
const copyFoodArray = (food = ['milk', 'bread', 'apple']) => {
  const foodCopy = [];
  food.forEach((n) => foodCopy.push(n));
  return foodCopy;
};
...
const findHigherSalariedEmployees = (employees = [{ name: '', salary: 200000 }]) => {
  ...
  employees.forEach((employe) => {
    if (employe.salary > 300000) {
      peopleRich.push(employe);
    }
  });
  return peopleRich;
};
...
const findEmployeesFromMarketing = (employees = [{ name: '', salary: 200000, dep: 'marketing' }]) => {
  const peopleRichMark = [];
  employees.forEach((employe) => {
    if (employe.salary > 300000 && employe.dep.includes('marketing')) {
      peopleRichMark.push(employe);
    }
  });
  return peopleRichMark;
};
...
const findGermanMigrants = (citizens = [{ nation: 'German', email: 'franz@gmail.com', city: 'Budapest' }]) => {
  const peopleGermanGmail = [];
  citizens.forEach((citizen) => {
    if (citizen.nation.includes('German') && citizen.email.includes('gmail') && citizen.city.includes('Budapest')) {
      peopleGermanGmail.push(citizen);
    }
  });
  return peopleGermanGmail;
};
