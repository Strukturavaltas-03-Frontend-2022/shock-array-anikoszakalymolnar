 ...
 */
const sortFood = (food = ['milk', 'bread', 'apple']) => food.sort();
 ...
 * @returns {string[]} - a string alapján rendezett tömb.
 */
const sortNames = (names = ['Bill', 'Anna']) => names.sort();
 ...
 */
const sortVariousNumbers = (prices = [121, 41, 56, 1245]) => prices.sort((a, b) => a - b);
...
const sortInternationalNames = (names = ['Árpád', 'Beáta', 'Örs', 'Zoltán']) => {
  names.sort((string1, string2) => string1.localeCompare(string2));
  return names;
};
...
const sortInternationalUsers = (users = [{ name: 'Ákos', age: 44, city: 'Bp' }]) => {
  users.sort((user1, user2) => user1.name.localeCompare(user2));
  return users;
};
...
const sortInternationalUsersByCity = (users = [{ name: 'Ákos', age: 44, city: 'Bp' }]) => {
  users.sort((user1, user2) => user1.city.localeCompare(user2.city));
  return users;
};
