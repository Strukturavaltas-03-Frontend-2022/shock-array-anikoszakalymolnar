 ...
 */
const fourCharFoods = (foods = ['milk', 'bread', 'apple']) => foods.filter((food) => food.length === 4);
 ...
 */
const findNamesWithA = (names = ['Bill', 'Anna', 'Andy']) => names.filter((name) => name.toLowerCase().includes('a'));
...
const findBigPrices = (prices = [75, 80, 56, 1245]) => {
  const filteredPrices = prices.filter((price) => price * 1.27 > 100);
...
const filterOdds = (prices = [121, 41, 56, 1245]) => {
  const oddNumber = prices.filter((price) => price % 2 !== 0);
  return oddNumber;
};
...
const filterOldCars = (cars = [{ man: 'Ford', year: 2000, type: 'fiesta' }]) => {
  ...
  const oldCars = cars.filter((car) => (currentYear - car.year) > 10);
  return oldCars;
};
...
const findAdultCitizens = (citizens = [{ id: '33DDffgz', bd: 1987, nation: 'Hungarian' }]) => {
  const currentYear = new Date().getFullYear();
  const adultYear = citizens.filter((citizen) => ((currentYear - citizen.bd) > 18) && (citizen.nation.includes('Hungarian')));
  return adultYear;
};
...
const filterDietFoods = (dishes = [{ id: 1, kcal: 101, type: 'dessert' }]) => {
  const desserts = dishes.filter((dish) => (dish.type.includes('dessert') && (dish.kcal < 100)));
  return desserts;
};
