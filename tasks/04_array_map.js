...
const transformTitleCase = (foods = ['milk', 'bread', 'apple']) => {
  const capitalLetter = foods.map((food) => food[0].toUpperCase() + food.slice(1));
...
const addNamePrefixToStudents = (students = [{ lastName: 'Doe', gender: 'male' }]) => {
  const prefName = students.map((elem) => {
    const student = elem;
...
const getSellPrices = (prices = [75, 80, 56, 1245]) => {
  const priceBig = prices.map((price) => (price * 1.3));
  return priceBig;
};
...
const getStudentReports = (students = [{ firstName: 'John', lastName: 'Doe', points: 20 }]) => {
  const result = students.map((student) => `${student.firstName} ${student.lastName}, points: ${student.points}.`);
  return result;
};
...
const addStudentReports = (students = [{ firstName: 'John', points: 20 }]) => {
  const passExam = students.map((student) => {
    const data = student;
    if (data.points > 50) {
      data.result = 'passing';
    } else {
      data.result = 'not passing';
    }
    return data;
  });
  return passExam;
...
const addCorrectedIncome = (employees = [{
  name: 'Bob', salary: 200000, premium: 1000, corr: 0.2,
}]) => {
  const extraMoney = employees.map((employee) => {
    const data = employee;
    data.income = (data.salary + data.premium) * (1 + data.corr);
    return data;
  });
  return extraMoney;
};
...
const addDishPrices = (dishes = [{ id: 1, kcal: 101, type: 'dessert' }]) => {
  const priceFood = dishes.map((dish) => {
    const typeOfFood = dish;
    if (typeOfFood.type.includes('dessert')) {
      typeOfFood.price = Math.round(200 - (typeOfFood.kcal * 1.5));
    } else {
      typeOfFood.price = Math.round(25 + typeOfFood.kcal);
    }
    return typeOfFood;
  });
  return priceFood;
